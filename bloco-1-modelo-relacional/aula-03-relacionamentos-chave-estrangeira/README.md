...
1. **`ex01.md`** — para cada par abaixo, faça as **duas perguntas no plural** da seção 3, escreva as duas respostas e classifique em 1:1, 1:N ou N:M: (a) `ALUNO` e `EMPRESTIMO`; (b) `OBRA` e `EXEMPLAR`; (c) `ALUNO` e `DISCIPLINA`; (d) `EXEMPLAR` e `EMPRESTIMO`. Para cada 1:N, diga em qual das duas tabelas a **FK** vai morar. *Confira assim: são três 1:N e um N:M. Em todo 1:N a FK fica do lado que tem "um só" do outro.*
2. **`ex02.md`** — o esquema abaixo está errado. Ele quer registrar que **uma obra tem vários exemplares**:
   `OBRA(isbn, titulo, tombo_exemplar)`
   Explique **o que acontece** quando a biblioteca compra o segundo exemplar da mesma obra, e escreva os dois esquemas corrigidos. *Confira assim: no modelo corrigido, cadastrar o décimo exemplar de uma obra não pode exigir alterar nenhuma linha de `OBRA`.*
3. **`ex03.md`** — a biblioteca tem clubes de leitura e precisa registrar **quais alunos participam de quais clubes**, guardando a data em que cada aluno entrou. Escreva os **três esquemas**, sublinhe as PKs e indique as FKs. *Confira assim: a data de entrada não cabe em `ALUNO`, que entra em vários clubes em datas diferentes, nem em `CLUBE` — ela mora na terceira tabela.*
