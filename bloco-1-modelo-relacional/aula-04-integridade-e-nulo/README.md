...
1. **`ex01.md`** — para cada situação, diga **qual restrição de integridade** é violada (domínio, entidade, referencial ou semântica): (a) inserir um empréstimo para a matrícula 9999999, que não existe; (b) inserir uma obra com `ano_publicacao = 'antigo'`; (c) inserir dois exemplares com o mesmo tombo; (d) inserir um exemplar sem tombo; (e) registrar devolução anterior à retirada; (f) emprestar um exemplar cuja situação é `extraviado`; (g) inserir uma disciplina com `carga_horaria = -40`. *Confira assim: as quatro restrições aparecem, e a semântica aparece mais de uma vez — ela é a que sobra quando nenhuma das outras três serve.*
2. **`ex02.md`** — retome o modelo da clínica que você fez no `ex03` da Aula 01 e escreva o **domínio de cada atributo** em português: tipo e faixa de valores válidos. Depois marque quais atributos são obrigatórios e quais aceitam vazio, com um motivo escrito para cada vazio permitido. *Confira assim: se algum atributo ficou opcional sem motivo escrito, ele não deveria ser opcional.*
3. **`ex03.md`** — o modelo da Biblioteca tem estas seis chaves estrangeiras: `EMPRESTIMO→ALUNO`, `EMPRESTIMO→EXEMPLAR`, `EXEMPLAR→OBRA`, `TELEFONE→ALUNO`, `RENOVACAO→EMPRESTIMO`, `MULTA→EMPRESTIMO`. Escolha a **ação referencial** de cada uma e justifique em uma linha. Em seguida aponte qual delas seria a mais destrutiva se declarada "em cascata" por engano e descreva o estrago em uma frase. *Confira assim: para cada cascata que você escolheu, a linha dependente tem que ser incapaz de significar algo sozinha.*
4. **`ex04.md`** — uma tabela de funcionários tem a coluna `data_demissao` vazia em 380 das 400 linhas. (a) Qual dos três significados de nulo é esse? (b) Escreva a frase que documenta esse nulo no modelo; (c) o RH pediu um relatório de "quem já saiu da empresa" e recebeu zero linhas, embora 20 pessoas tenham saído — explique o que aconteceu; (d) proponha uma mudança no modelo que tornaria esse erro impossível. *Confira assim: o item (c) tem a ver com a tabela de verdade da seção 6, e o item (d) não é "trocar nulo por zero".*
5. **Desafio 🌶️ `ex05.md`** — projete um esquema pequeno (4 tabelas) em que uma exclusão em cascata mal declarada apague dados que ninguém queria perder. Entregue: o esquema com as FKs e suas ações, a exclusão de **uma linha só**, a lista do que desaparece junto, e o esquema corrigido. Depois escreva as **três perguntas** que você vai fazer, daqui em diante, sempre, antes de declarar uma cascata. *Confira assim: a exclusão precisa ser inocente — uma linha de uma tabela que parecia inofensiva — e o estrago precisa alcançar pelo menos três tabelas.*
